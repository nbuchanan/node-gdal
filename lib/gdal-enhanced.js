...
var gdal = require('./binding/node-v46-win32-x64/gdal-enhanced.node'),
// var gdal = require('gdal-enhanced'),
// var gdal = require('./gdal.js'),
// var gdal = require('C:/git/github/node-gdal/lib/binding/node-v46-win32-x64/gdal-enhanced.node'),
    fs = require('fs'),
    path = require('path'),
    logger = require('loglevel').noConflict().getLogger('gdal-enhanced');

logger.setLevel('DEBUG');

var processDataset = function(ds) {
...
if (require.main === module) {
    ...
    var argv = require('yargs')
        ...
        .command('info', 'Attempt to get geospatially relevant metadata for a file using GDAL')
        .example(baseAppName + ' info -f back40.tif', 'Print metadata for a GeoTIFF')
        .alias('f', 'file')
        .nargs('f', 1)
        ...
        .help('h')
        .alias('h', 'help')
        .epilog('copyright 1984')
        .argv;

    if (argv.drivers) {
        // List currently configured drivers
        logger.debug('Currently supported GDAL drivers: ' + gdal.drivers.getNames());
    }
    ...
    logger.debug('gdal-enhanced required as a module');
}